...
function PlayerMoney.Increment(player, amount)
	...
	if PlayerMoney.Values[player] < 0 and not NEGATIVE_MONEY then
		PlayerMoney.Values[player] = 0
	end
...
function PlayerMoney.Set(player, setTo)
	if PlayerMoney.Values[player] == nil then return end
	PlayerMoney.Values[player] = if NEGATIVE_MONEY then setTo else math.max(0, setTo)
	if LEADERSTATS_ENABLED then
		updateLeaderstats(player)
	end
end
...
local function onPlayerAdded(player)
	...
	PlayerMoney.Values[player] = DEFAULT_AMOUNT
